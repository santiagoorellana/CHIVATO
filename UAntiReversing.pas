...
function CreateNameFor(init: String; str: String): String;  //Obtiene un nombre único de 32 a partir de una cadena cualquiera.

//Funciones simples programadas de forma compleja.
function ReturnTrue(A: Boolean; B: Boolean; C: Boolean): Boolean; overload;
function ReturnTrue(A: Boolean; B: Boolean; C: Integer): Boolean; overload;
function ReturnTrue(A: String; B: Integer; C: Integer): Boolean; overload;
function ReturnFalse(A: Boolean; B: Boolean; C: Boolean): Boolean; overload;
function ReturnFalse(A: Boolean; B: Boolean; C: Integer): Boolean; overload;
function ReturnFalse(A: String; B: Integer; C: Integer): Boolean; overload;

//Devuelve la cantidad de dispositivos que se pueden registrar
//cuando el software CHIVATO no tiene licencia válida.
function GetUnlicenceMaxDevices: Integer;

implementation

uses UDateID, USHA256, URC4;
...
//-----------------------------------------------------------------------------
function CreateNameFor(init: String; str: String): String;
var n: Integer;
    hash: String;
begin
Result := '';
if str = '' then Exit;                //Sale si la cadena STR está vacía.
hash := Sha256(str);                  //Calcula el Sha256 de la cadena str.
Result := init + hash;
end;
...
//-----------------------------------------------------------------------------
// Funciones que siempre devuelven el mismo valor.
//-----------------------------------------------------------------------------

//-----------------------------------------------------------------------------
// Siempre devuelven TRUE.
//
// Se construye sobre la base de una función booleana que siempre devuelve
// TRUE. La tabla de Carnaut es la siguiente, la cual permite hallar la
// ecuación canónica para la función booleana deseada.
//
// A B C R
...
//
// Entonces la función booleana es:
// F = (not A and not B and not C) or (not A and not B and C) or
//     (not A and B and not C) or (not A and B and C) or
//     (A and not B and not C) or (A and not B and C) or
//     (A and B and not C) or (A and B and C)
//
// Todo el código restante es basura que sin importar que operaciones
// realiza la función siempre devolverá TRUE.
//-----------------------------------------------------------------------------
function ReturnTrue(A: Boolean; B: Boolean; C: Boolean): Boolean; overload;   //Siempre devuelve TRUE.
begin
Result := (not A and not B and not C) or (not A and not B and C) or
          (not A and B and not C) or (not A and B and C) or
          (A and not B and not C) or (A and not B and C) or
          (A and B and not C) or (A and B and C);
end;

function ReturnTrue(A: Boolean; B: Boolean; C: Integer): Boolean; overload;               //Siempre devuelve TRUE.
var I: Boolean;
begin
I := ((C > 32) or (A and B)) and ((C < 128) or (A and not B));
Result := ReturnTrue(A, B, I);
end;

function ReturnTrue(A: String; B: Integer; C: Integer): Boolean; overload;                            //Siempre devuelve TRUE.
var I, J, K: Boolean;
begin
K := Length(A) > 0;
J := (B <= 512) and (B >= 256);
I := ((C > 32) or (K and J)) and ((C < 128) or (K and not J));
Result := ReturnTrue(I, J, K);
end;
...
//
// Se construye sobre la base de una función booleana que siempre devuelve
// FALSE. La tabla de Carnaut es la siguiente, la cual permite hallar la
// ecuación canónica para la función booleana deseada.
//
// A B C R
// 0 0 0 0 = (A or B or C)
// 0 0 1 0 = (A or B or not C)
// 0 1 0 0 = (A or not B or C)
// 0 1 1 0 = (A or not B or not C)
// 1 0 0 0 = (not A or B or C)
// 1 0 1 0 = (not A or B or not C)
// 1 1 0 0 = (not A or not B or C)
// 1 1 1 0 = (not A or not B or not C)
//
// Entonces la función booleana es:
// F = (A or B or C) and (A or B or not C) and
//     (A or not B or C) and (A or not B or not C) and
//     (not A or B or C) and (not A or B or not C) and
//     (not A or not B or C) and (not A or not B or not C)
//
// Todo el código restante es basura que sin importar que operaciones
// realiza la función siempre devolverá FALSE.
//-----------------------------------------------------------------------------
function ReturnFalse(A: Boolean; B: Boolean; C: Boolean): Boolean; overload;  //Siempre devuelve FALSE.
begin
Result := (A or B or C) and (A or B or not C) and
          (A or not B or C) and (A or not B or not C) and
          (not A or B or C) and (not A or B or not C) and
          (not A or not B or C) and (not A or not B or not C);
end;

function ReturnFalse(A: Boolean; B: Boolean; C: Integer): Boolean; overload;              //Siempre devuelve FALSE.
var I: Boolean;
begin
I := ((C > 32) or (A and B)) and ((C < 128) or (A and not B));
Result := ReturnFalse(A, B, I);
end;

function ReturnFalse(A: String; B: Integer; C: Integer): Boolean; overload;                           //Siempre devuelve FALSE.
var I, J, K: Boolean;
begin
K := Length(A) > 0;
J := (B <= 512) and (B >= 256);
I := ((C > 32) or (K and J)) and ((C < 128) or (K and not J));
Result := ReturnFalse(I, J, K);
end;
...
// cuando el software CHIVATO no tiene licencia válida.
//
// Siempre devuelve 50. Lo hace complicadamente para evitar guardar el número
// directamente en el fuente, para evitar así que lo puedan cambiar.
//-----------------------------------------------------------------------------
function GetUnlicenceMaxDevices: Integer;
var a, b: Integer;
begin
a := Random(9998) + 2;           //Crea un número mayor o igual que 2 y menor igual que 10000. 
b := a div 2;                     //Le calcula la mitad.
Result := Round((b / a) * 100);   //Devuelve el porciento que representa la mitad. Siempre 50%.
end;
