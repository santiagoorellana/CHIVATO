...
const cAppTitle       = '0P1ITJ8W;BLU=D6';                                                        //Nombre de la aplicación = 'CHIVATO';
const cAppID          = 1;                                                                        //Identificador de la aplicación.
const cAppVersion     = 1;                                                                        //Versión de la aplicación.
const cAppSubVersion  = 3;                                                                        //Subversión de la aplicación.
...
//-----------------------------------------------------------------------------
const cFileInit = '0';   //Este es el programa número 0.
...
var Writer: TStrings;
begin
Result := ReturnFalse(FileName, 127, 3);                    //Por defecto devuelve FALSE. Los parámetros son para confundir.
try
   ...
   try
      ...
      Result := ReturnTrue(FileName, 255, 3);               //Siempre devuelve TRUE. Los parámetros son para confundir.
   except
...
//-----------------------------------------------------------------------------
// Cierra una aplicación por el nombre de su fichero ejecutable.
//-----------------------------------------------------------------------------
function KillTask(FileName: String): Integer;
...
// o TeraByte según la cantidad del valor.
//
// Entrada:
// GB = Cantidad de información en Giga Bytes.
//-----------------------------------------------------------------------------
function ToXByte(GB: Double): String;
...
//-----------------------------------------------------------------------------
// Obtiene el número de serie del dispositivo indicado.
//-----------------------------------------------------------------------------
function GetDiskSerial(DiskChar: Char): DWORD;
...
try                                                   //Obtiene el número de serie y nombre del dispositivo de almacenamiento.
   if not GetVolumeInformation(PChar(SystemDir),      //Path del disco de sistema.
                               nil,                   //No lo usamos.
                               0,                     //No lo usamos.
                               @FDiskSerial,          //Aquí se coloca el número de serie.
                               MaxFNLength,           //No lo usamos.
                               VolFlags,              //No lo usamos.
                               nil,                   //No lo usamos.
                               0                      //No lo usamos.
                               )then                  //Si ocurre algún error
      Result := 0                                     //devuelve cero...
   else                                               //de lo contrario, devuelve
      Result := FDiskSerial;                          //el número e serie.
except
   Result := 0;
end;
end;
