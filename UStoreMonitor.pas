...
uses Windows, SysUtils, Classes, UAntiReversing, UCommon;
...
Type
  TStoreInfo = record
    ...
    sIgnore: Boolean;             //TRUE si el dispositivo debe ser ignorado.
  end;
...
type
  ...
  private
    ...
    procedure UnregisterAndReportAllStores;               //Simula la desconexión de los dispositivos de almacenamiento.
    ...
    procedure ClearStoresList;
  protected
    procedure Execute; override;
  public
    constructor Create;                                                             //Inicia el objeto monitor.
    destructor Destroy; override;                                                   //Prepara el objeto para eliminarlo.
    property Interval: Integer read FIntervalMS write FIntervalMS;                  //Determian el intervalo en milisegundos entre búsquedas.
    ...
    procedure IgnorePresentDevices;
  end;
...
begin
inherited Create(True);       //Llama al constructor de la clase paterna.
ClearStoresList;              //Borra la lista de dispositivos.
...
//-----------------------------------------------------------------------------
procedure TStoreMonitor.Execute; 
begin
while not Terminated do                     //Repite contínuamente las búsquedas...
      begin
      Synchronize(OneSearch);               //Ejecuta una búsqueda de dispositivos de almacenamiento.
      Sleep(FIntervalMS);                   //Para que no interfiera en el funcionaiento del dispositivo.
      OnEventEndSearch;                     //Notifica que se ha termina un ciclo de búsqueda.
      end;
Synchronize(UnregisterAndReportAllStores);  //Guarda todos los datos de los dispositivos registrados.
end;
...
//-----------------------------------------------------------------------------
// Salva todo lo que está en la lista de dispositivo.
// Lo que hace es llamar al evento OnRemove por cada dispositivo de la
// lista que se encuentre conetado. Esto se hace antes de cerrar el
// programa, para que no se dejen de reportar los dispositivos que aún
// están conectados.
//-----------------------------------------------------------------------------
procedure TStoreMonitor.UnregisterAndReportAllStores;
var n: Integer;
    Store: TStoreInfo;
begin
try
   ...
   FStoresMask := 0;                              //Reinicia la máscara de dispositivos detectados.
except
end;
end;

//-----------------------------------------------------------------------------
// Marca los dispositivos presentes para que sean ignorados.
//-----------------------------------------------------------------------------
procedure TStoreMonitor.IgnorePresentDevices;
var n: Integer;
begin
try
   for n := 2 to cLastDevice do                 //Recorre la lista de dispositivos.
       if FStoresMask and (1 shl n) > 0 then    //Si está CONECTADO el dispositivo n...
          FStores[n].sIgnore := True;           //Ignora el dispositivo.
except
end;
end;
...
//-----------------------------------------------------------------------------
procedure TStoreMonitor.ClearStoresList;
var n: Integer;
begin
try
   for n := 2 to cLastDevice do     //Recorre la lista de dispositivos.
       UnregisterStore(n);          //Borra los datos de registro del dispositivo.
   FStoresMask := 0;                //Reinicia la máscara de dispositivos detectados.
except
end;
end;
...
begin
if FReportDevices then                          //Si el detector está activado...
   if not Store.sIgnore then    //Si no se ignora...
...
try
   ...
   //Si se pudo obtener el número de serie y nombre del dispositivo de almacenamiento...
   if RGVI then
      begin
